...
class Firebase {
  ...
  doRegisterOrder = (paymentInfo, order_description) => {
    ...
    this.db
      ...
      .set({
        content:
          "Welcome! As soon as a booster take your order, you will be able to contact him using this chat",
  ...
  doSetCredentials = (username, password, summoner, order_id) => {
    this.db
      .collection("orders")
      .doc(order_id)
      .update({
  ...
  getTimeline = (order_id, fn) => {
    return this.db.collection("timelines").doc(order_id).onSnapshot(fn);
  };
